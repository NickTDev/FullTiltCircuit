...
public class Wiimote
{
    ...
    public BalanceBoardData BalanceBoard
    {
        get
        {
            if (current_ext == ExtensionController.BALANCE_BOARD)
    ...
    private BalanceBoardData.BalanceBoardCalabrationData? balanceBoardCalabrationData = null;
    ...
    private void RespondIdentifyExtension(byte[] data)
    {
        ...
        {
            _current_ext = ExtensionController.BALANCE_BOARD;
            if (_Extension == null || _Extension.GetType() != typeof(BalanceBoardData))
            {
                    // TODO: Get Calabration Data 
                //// Get Calabration Info 
                //SendRegisterReadRequest(RegisterType.CONTROL, 0xa40024, 24, RespondCalibrationRead);
                //if (balanceBoardCalabrationData == null)
                //    Debug.LogError("CalabrationData not read correctly!");

                _Extension = new BalanceBoardData(this);
            }

            SendDataReportMode(InputDataType.REPORT_BUTTONS_EXT19); // We need to tell the board how to send info 
        }
    ...
    private void RespondCalibrationRead(byte[] data)
    {
        balanceBoardCalabrationData = new BalanceBoardData.BalanceBoardCalabrationData(data);
    }
